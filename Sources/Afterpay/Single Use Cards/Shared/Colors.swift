//
//  Colors.swift
//  Afterpay
//
//  Created by Nabila Herzegovina on 15/2/21.
//  Copyright © 2021 Afterpay. All rights reserved.
//

import Foundation
import UIKit

extension UIColor {

  static let primaryColor = UIColor(red: 0.698, green: 0.988, blue: 0.894, alpha: 1)

  static let singleUseCardDescriptionTextColor = UIColor(red: 0.7, green: 0.7, blue: 0.7, alpha: 1)
  static let singleUseCardTextColor = UIColor(red: 0.8, green: 0.8, blue: 0.8, alpha: 1)

  static let termsConditionTitleColor  = UIColor(red: 0.4, green: 0.4, blue: 0.4, alpha: 1)
}
